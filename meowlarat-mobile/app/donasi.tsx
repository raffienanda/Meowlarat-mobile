...
// Update Interface biar support Status & Tanggal
interface DonasiLog {
  id: number;
  username: string;
  nominal: number; // Backend kirim 'amount' atau 'nominal'? Sesuaikan fieldnya.
  amount?: number; // Jaga-jaga kalau backend pakai amount
  pesan: string;
  message?: string; // Jaga-jaga kalau backend pakai message
  date?: string;
...
export default function DonasiScreen() {
  ...
  // Fetch donasi setiap kali filter berubah
  useEffect(() => {
    fetchDonations();
  }, [filterType, userSession]); // Trigger saat filter ganti atau session loaded
  ...
  const fetchDonations = async () => {
    // Kalau pilih 'Saya' tapi belum login, jangan fetch
    if (filterType === 'mine' && !userSession) return;

    setLoading(true);
    try {
      let endpoint = `${API_URL}/api/donasi`;
      
      // Jika filter 'Saya', ganti endpoint ke history user
      if (filterType === 'mine' && userSession?.username) {
        endpoint = `${API_URL}/api/donasi/history/${userSession.username}`;
      }

      const response = await fetch(endpoint);
      const res = await response.json();
      
      // Handle format response yang mungkin beda (array langsung vs object)
      let dataList = [];
      if (Array.isArray(res)) {
        dataList = res;
      } else if (res.status === 'success' || res.data) {
        dataList = res.data;
        if(res.total) setTotalDonasi(res.total); // Update total cuma kalau ambil All
      }

      setDonations(dataList);
      
      // Hitung total manual jika endpoint history tidak balikin total global
      if (filterType === 'all' && Array.isArray(res)) {
         const total = res.reduce((acc: number, curr: any) => acc + (curr.nominal || curr.amount || 0), 0);
         setTotalDonasi(total);
      }

    } catch (error) {
      console.log("Gagal ambil history donasi", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // @ts-ignore
      formData.append('img', { uri: image, name: filename, type }); 

      // NOTE: Sesuaikan endpoint ini kalau backend kamu pakai /api/donasi (POST)
      ...
      if (response.ok) {
        ...
        setFilterType('mine'); // Pindah ke tab Saya biar user liat statusnya
        fetchDonations(); 
      } else {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
      <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        {/* FORM TETAP ADA DI ATAS (TIDAK BERUBAH) */}
        <View style={styles.formContainer}>
            ...
            <TextInput style={[styles.input, styles.textArea]} placeholder="Tulis pesan..." multiline numberOfLines={3} value={pesan} onChangeText={setPesan} />

            <Text style={styles.label}>Metode Pembayaran</Text>
            {loading && metodes.length === 0 ? <ActivityIndicator color={Colors.primary} /> : (
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.methodScroll}>
                    {metodes.map((m) => {
                        const isActive = m.isActive !== false; 
                        return (
                            <TouchableOpacity key={m.id} 
                                style={[styles.methodCard, selectedMetode === m.id && styles.methodCardActive, !isActive && styles.methodCardDisabled]}
                                onPress={() => isActive ? setSelectedMetode(m.id) : Alert.alert("Info", "Metode belum tersedia.")} disabled={!isActive}>
                                <Text style={[styles.methodText, selectedMetode === m.id && styles.methodTextActive, !isActive && styles.methodTextDisabled]}>{m.nama}</Text>
                            </TouchableOpacity>
                        );
                    })}
                </ScrollView>
            ...
            {currentMetode && (
                <View style={styles.rekInfo}>
                    <Text style={styles.rekTitle}>Transfer ke:</Text>
                    {currentMetode.logo && (currentMetode.category === 'E-Wallet' || currentMetode.nama.includes('QRIS')) ? (
                        <View style={styles.qrisWrapper}>
                            <Image source={{ uri: `${API_URL}/uploads/logo/${currentMetode.logo}` }} style={styles.qrisImage} resizeMode="contain" />
                        </View>
                    ) : null}
                    {currentMetode.rek && <Text style={styles.rekNumber} selectable>{currentMetode.rek}</Text>}
            ...
            <TouchableOpacity style={styles.uploadBtn} onPress={pickImage}>
                ...
                <Text style={styles.uploadText}>{image ? "Ganti Gambar" : "Upload Bukti"}</Text>
            </TouchableOpacity>
            {image && <Image source={{ uri: image }} style={styles.previewImage} resizeMode="contain" />}
        ...
        {/* BAGIAN BAWAH: TRANSPARANSI & FILTER */}
        <View style={styles.transparencyContainer}>
            ...
            {/* HEADER TABEL DENGAN FILTER TOMBOL */}
            <View style={styles.filterHeader}>
                <Text style={styles.historyTitle}>Riwayat Donatur</Text>
                ...
                <View style={styles.filterBtnContainer}>
                    ...
                    <TouchableOpacity 
                        ...
                        onPress={() => {
                            if(!userSession) Alert.alert("Login", "Silakan login dulu.");
                            else setFilterType('mine');
                        }}
                    >
                        <Text style={[styles.filterText, filterType === 'mine' && styles.filterTextActive]}>Saya</Text>
                    </TouchableOpacity>
                </View>
            </View>
            
            <View style={styles.tableHeader}>
                <Text style={[styles.colHeader, {flex: 3}]}>Nama / Pesan</Text>
                <Text style={[styles.colHeader, {flex: 2, textAlign: 'right'}]}>Nominal</Text>
            </View>

            {loading ? <ActivityIndicator style={{marginTop:20}} /> : donations.length === 0 ? (
                <Text style={{textAlign:'center', marginTop: 20, color:'#999'}}>
                    {filterType === 'all' ? 'Belum ada donasi.' : 'Kamu belum pernah donasi.'}
                </Text>
            ) : (
                donations.map((item) => (
                    <View key={item.id} style={styles.tableRow}>
                        <View style={{flex: 3}}>
                            ...
                            {(item.pesan || item.message) && (item.pesan !== '-' && item.message !== '-') && (
                                <Text style={styles.rowMsg} numberOfLines={1}>"{item.pesan || item.message}"</Text>
                            )}
                        </View>

                        <Text style={[styles.rowNominal, {flex: 2, textAlign: 'right'}]}>
                            {formatRupiah(item.nominal || item.amount || 0)}
                        </Text>
                    </View>
                ))
            )}
...
const styles = StyleSheet.create({
  ...
  formContainer: {
    backgroundColor: '#fff', marginTop: -30, marginHorizontal: 15, borderRadius: 15, padding: 20,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 5, elevation: 3, marginBottom: 20
  },
  ...
  methodCard: { paddingVertical: 12, paddingHorizontal: 20, backgroundColor: '#f0f2f5', borderRadius: 16, marginRight: 10, borderWidth: 1, borderColor: '#eee', minWidth: 100, alignItems: 'center', justifyContent: 'center' },
  ...
  qrisWrapper: { width: '100%', alignItems: 'center', marginVertical: 15, backgroundColor: '#fff', borderRadius: 10, paddingVertical: 10, borderWidth: 1, borderColor: '#eee' },
  ...
  totalCard: { backgroundColor: Colors.primary, borderRadius: 12, padding: 20, alignItems: 'center', marginBottom: 25, elevation: 4 },
  ...
  filterBtnContainer: { flexDirection: 'row', backgroundColor: '#e0e0e0', borderRadius: 20, padding: 2 },
  filterBtn: { paddingVertical: 6, paddingHorizontal: 15, borderRadius: 18 },
